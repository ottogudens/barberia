                            ...
                            <table class="table table-bordered">
                                ...
                                <tbody>
                                    ...
                                    $stmt = $con->prepare("SELECT a.*, c.first_name as c_name, c.last_name as c_lname, e.first_name as e_name, e.last_name as e_lname,
                                                               (SELECT COALESCE(SUM(s.service_price), 0) FROM services s JOIN services_booked sb ON s.service_id = sb.service_id WHERE sb.appointment_id = a.appointment_id) as calc_price,
                                                               (SELECT STRING_AGG(s.service_name, ', ') FROM services s JOIN services_booked sb ON s.service_id = sb.service_id WHERE sb.appointment_id = a.appointment_id) as service_names
                                                        FROM appointments a 
                                    ...
                                    foreach ($unpaid_appts as $appt):
                                        $price = $appt['calc_price'];
                                        ?>
                                        <tr>
                                            <td><?php echo $appt['start_time']; ?></td>
                                            <td><?php echo htmlspecialchars($appt['c_name'] . ' ' . $appt['c_lname']); ?></td>
                                            <td><?php echo htmlspecialchars($appt['e_name'] . ' ' . $appt['e_lname']); ?></td>
                                            <td>
                                                <?php echo htmlspecialchars($appt['service_names'] ?? ''); ?>
                                            </td>
                                ...
                                <tbody>
                                    ...
                                    foreach ($paid_appts as $appt):
                                        ...
                                        <tr>
                                            ...
                                            <td><?php echo htmlspecialchars($appt['c_name'] . ' ' . $appt['c_lname']); ?></td>
                                            <td><?php echo htmlspecialchars($appt['e_name'] . ' ' . $appt['e_lname']); ?></td>
                                            <td><?php echo htmlspecialchars($appt['payment_method']); ?></td>
                ...
                <form method="GET" class="form-inline mb-4">
                    ...
                    <select name="employee_id" class="form-control mr-3">
                        ...
                        while ($e = $stmtAllEmp->fetch()) {
                            ...
                            echo "<option value='" . $e['employee_id'] . "' $selected>" . htmlspecialchars($e['first_name'] . " " . $e['last_name']) . "</option>";
                        }
                        ?>
                    </select>
