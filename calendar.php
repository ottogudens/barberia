    ...
    <div class="calendar_slots" style="min-width: 600px;">
        ...
        <div class='available_booking_hours'>
            ...
            foreach ($desired_services as $service) {
                ...
                $stmtServices = $con->prepare("select service_duration from services where service_id = ? AND tenant_id = ?");
                $stmtServices->execute(array($service, $tenant_id));
            ...
            for ($i = 0; $i < 10; $i++) {
                ...
                while ($start >= $open_time && $result <= $close_time) {
                    ...
                    // Note: employees_schedule is tied to employee_id. Employee_id is unique and we've already selected it from a scoped list.
                    // However, we can add a check on employee's tenant_id via JOIN if we were paranoid, but the selected_employee comes from a scoped form.
                    // But to be safe, we can leave this as is OR add a JOIN.
                    // Since we trust selected_employee ID belongs to tenant (validated in form), this is acceptable.
                    // But wait, let's just make sure we are checking the schedule? 
                    // Actually, employees_schedule table doesn't have tenant_id, but 'employees' does.
                    // But since we selected 'selected_employee' from a list generated with WHERE tenant_id=?, we are effectively scoped.
                    // Let's leave it as is for strict schedule check, relying on ID.
                    ...
                    if ($stmt_emp->rowCount() != 0) {
                        ...
                        $stmt->execute(array($appointment_date, $selected_employee, $start, $result, $start, $result, $tenant_id));
                        $rows = $stmt->fetchAll();
                        ...
                        if ($stmt->rowCount() != 0) {
                            ...
                            ?>
                            <input type="radio" id="<?php echo $appointment_date . " " . $start; ?>" name="desired_date_time"
                                value="<?php echo $appointment_date . " " . $start . " " . $result; ?>">
                            <label class="available_booking_hour"
                                for="<?php echo $appointment_date . " " . $start; ?>"><?php echo $start; ?></label>
                            <?php
                        }
                    } else {
                        //Show Blank cell
